    ...
    public BufferedImage draw(BufferedImage image)
    {
        ...
        for (int i = 0; i < chunks.length; i++)
        {
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int y = 0; y < 16; y++)
                {
                    double image_x = (MainApplet.engine.protagonist.x_chunk - MainApplet.engine.map.chunks.get(chunks[i]).x) * (256 * x_multiplier) - (MainApplet.engine.protagonist.x * x_multiplier) + ((16 * x_multiplier) * x) + Integer.valueOf(GlobalProperties.global.getProperty("width")) / 2;
                    double image_y = (MainApplet.engine.protagonist.y_chunk - MainApplet.engine.map.chunks.get(chunks[i]).y) * (256 * y_multiplier) - (MainApplet.engine.protagonist.y * y_multiplier) + ((16 * y_multiplier) * y) + Integer.valueOf(GlobalProperties.global.getProperty("height")) / 2;
